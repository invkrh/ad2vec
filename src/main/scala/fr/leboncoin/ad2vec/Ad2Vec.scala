 ...
 */
abstract class Ad2Vec private (private var idCol: String,
  ...
  def setWeighted(weighted: Boolean): this.type = {
  ...
  def computeWeight(processed: DataFrame): DataFrame = {
    val tfidf = new HashingTFIDF("words", "tfidf", processed)
    tfidf.result()
  }

  def weightedVec(weightedDoc: DataFrame, wordVecIndex: Map[String, Vector]): DataFrame = {
